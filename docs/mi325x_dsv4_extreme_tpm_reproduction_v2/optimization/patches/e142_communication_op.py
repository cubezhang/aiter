...
def _use_new_ar_default() -> bool:
    value = os.environ.get("AITER_TP_ALLREDUCE_USE_NEW", "1").strip().lower()
    return value not in {"0", "false", "off", "no"}


def _resolve_use_new(use_new: Optional[bool] = None, shape_m: Optional[int] = None) -> bool:
    if use_new is not None:
        return use_new
    if not _use_new_ar_default():
        return False
    min_m = int(os.environ.get("AITER_TP_ALLREDUCE_M_MIN", "0") or 0)
    max_m = int(os.environ.get("AITER_TP_ALLREDUCE_M_MAX", "-1") or -1)
    if min_m <= 0 and max_m < 0:
        return True
    if shape_m is None:
        return True
    if min_m > 0 and shape_m < min_m:
        return False
    if max_m > 0 and shape_m > max_m:
        return False
    return True
...
def tensor_model_parallel_all_reduce(
    ...
    return get_tp_group().all_reduce(
        input_,
        _resolve_use_new(use_new, input_.shape[0] if input_.dim() > 0 else None),
        open_fp8_quant,
        prefill_support,
    )
